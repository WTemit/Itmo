...
public class Ask {
     ...
     */
    public static String askString(String prompt) throws AskBreak {
        String value;
        while (true) {
            ...
            try {
                ...
                if (scriptMode) {
                    throw new NoSuchElementException("Пустая строка в скрипте не допускается для этого поля.");
                }
                System.err.println("Ввод не может быть пустым.");
        ...
        while (true) {
            ...
            try {
                ...
                if (condition.test(value)) {
                    ...
                    if (scriptMode) throw new IllegalArgumentException("Значение '" + value + "' не прошло валидацию в скрипте: " + errorMsg);
                    System.err.println("Ошибка валидации: " + errorMsg);
                }
            } catch (NoSuchElementException | IllegalStateException e) {
                throw e;
            } catch (NumberFormatException e) {
                if (scriptMode) throw new NumberFormatException("Неверный формат числа в скрипте: '" + line + "'");
                System.err.println("Неверный формат числа. Введите целое число.");
            }
        }
    }
    ...
    private static Double askDoubleNullable(String prompt, java.util.function.Predicate<Double> condition, String errorMsg) throws AskBreak {
        ...
        while (true) {
            ...
            try {
                ...
                if (scriptMode) throw new NumberFormatException("Неверный формат числа в скрипте: '" + line + "'");
                System.err.println("Неверный формат числа. Введите число.");
            }
        }
    }
    ...
    private static <T extends Enum<T>> T askEnumNullable(String prompt, Class<T> enumClass, String names) throws AskBreak {
        ...
        while (true) {
            ...
            try {
                ...
                if (scriptMode) throw new IllegalArgumentException("Неверное значение enum в скрипте: '" + line + "'");
                System.err.println("Неверное значение. Выберите из списка или нажмите Enter.");
            }
        }
    }
        ...
        try {
            ...
            while (true) {
                ...
                try {
                    ...
                    if (scriptMode) throw new IllegalArgumentException("Годовой оборот в скрипте должен быть > 0, получено: " + annualTurnover);
                    System.err.println("Ошибка валидации: Годовой оборот должен быть > 0.");
                } catch (NumberFormatException e) {
                    if (scriptMode) throw new NumberFormatException("Неверный формат числа для годового оборота в скрипте: '" + line + "'");
    ...
    private static Location askTown() throws AskBreak {
        try {
            ...
            // Поле y не может быть null, askInteger не вернет null, но может выбросить исключение, которое будет поймано выше

            String name = askString("        Название города: ");
            // Поле name не может быть пустым, askString это гарантирует или выбросит исключение
    ...
    public static String askPassword(String prompt) throws AskBreak {
        Console console = System.console();
        if (console != null) {
            // We are in a real console, can use readPassword
            char[] passwordChars = console.readPassword(prompt);
            if (passwordChars == null) { // User pressed Ctrl+D
                throw new NoSuchElementException("Input stream closed.");
            }
            return new String(passwordChars);
        } else {
            // We are not in a real console (e.g., in an IDE run window)
